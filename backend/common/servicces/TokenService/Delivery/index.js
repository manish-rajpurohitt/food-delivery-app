...
tokenHekper.generateAuthToken = async (payload) => {
  return jwt.sign(payload, process.env.AUTH_SECRET_KEY_DELIVERY, { expiresIn: process.env.TOKEN_EXPIRE_TIME });
}
...
tokenHekper.validateUser = async (req, res, next) => {
  try {
    ...
    const decoded = jwt.verify(token, process.env.AUTH_SECRET_KEY_DELIVERY);
